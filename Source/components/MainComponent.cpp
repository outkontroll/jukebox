...
MainComponent::MainComponent ()
{
    ...
    addAndMakeVisible (infoStatus = new Label ("Status info label",
                                               TRANS("Status:")));
    infoStatus->setFont (Font (15.00f, Font::plain));
    infoStatus->setJustificationType (Justification::centredLeft);
    infoStatus->setEditable (false, false, false);
    infoStatus->setColour (TextEditor::textColourId, Colours::black);
    infoStatus->setColour (TextEditor::backgroundColourId, Colour (0x00000000));
...
void MainComponent::resized()
{
    ...
    infoStatus->setBounds (208, 8, 56, 24);
    lblStatus->setBounds (264, 8, 264, 24);
...
bool MainComponent::keyPressed (const KeyPress& key)
{
    ...
    juce_wchar keyText = key.getTextCharacter();
    ...
    {
        creditManager->insert50();
        lblCredits->setText(String(creditManager->getCredit()), dontSendNotification);
        lblStatus->setText(String("---"), dontSendNotification);
    }
    else if(keyText == 'p')
    {
        creditManager->insert100();
        lblCredits->setText(String(creditManager->getCredit()), dontSendNotification);
        lblStatus->setText(String("---"), dontSendNotification);
    }
    else if(keyText == 'l')
    {
        creditManager->insert200();
        lblCredits->setText(String(creditManager->getCredit()), dontSendNotification);
        lblStatus->setText(String("---"), dontSendNotification);
    }
    else if(keyText == 's')
    {
        if(creditManager->onStartPlaySong())
        {
            lblCredits->setText(String(creditManager->getCredit()), dontSendNotification);
        }
        else
        {
            lblStatus->setText(String("Nincs eleg kredit a szam lejatszasahoz!"), dontSendNotification);
        }
    }
    else if(keyText == 'a')
    {
        if(creditManager->onStartPlayAlbum())
        {
            lblCredits->setText(String(creditManager->getCredit()), dontSendNotification);
        }
        else
        {
            lblStatus->setText(String("Nincs eleg kredit az album lejatszasahoz!"), dontSendNotification);
        }
    }
    else
    {
        //lblCredits->setText(String(++number), dontSendNotification);
    }
...
void MainComponent::setCreditManager(std::shared_ptr<ICreditManager> creditMgr)
{
...
<JUCER_COMPONENT documentType="Component" className="MainComponent" componentName=""
  ...
  <LABEL name="credits label" id="559731314e5f9fe6" memberName="lblCredits"
         ...
         bold="0" italic="0" justification="33"/>
  <LABEL name="Status info label" id="c9a6cc72c3279f87" memberName="infoStatus"
         virtualName="" explicitFocusOrder="0" pos="208 8 56 24" edTextCol="ff000000"
         edBkgCol="0" labelText="Status:" editableSingleClick="0" editableDoubleClick="0"
         focusDiscardsChanges="0" fontname="Default font" fontsize="15"
         bold="0" italic="0" justification="33"/>
  <LABEL name="Status label" id="e135f4a1f2d14bd" memberName="lblStatus"
         virtualName="" explicitFocusOrder="0" pos="264 8 264 24" edTextCol="ff000000"
